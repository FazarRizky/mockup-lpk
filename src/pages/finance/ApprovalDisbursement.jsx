...
export default function ApprovalDisbursement() {
  ...
  // FIX: gunakan useMemo biar tidak recalculation terus
  const totalPending = useMemo(
    () => requests.reduce((a, r) => a + r.jumlah, 0),
    []
  );

  const totalApproved = useMemo(
    () => approved.reduce((a, r) => a + r.jumlah, 0),
    []
  );

  const totalRejected = useMemo(
    () => rejected.reduce((a, r) => a + r.jumlah, 0),
    []
  );
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Review dan setujui pengajuan pencairan dana
        </p>
      </div>
            ...
            >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-0.5">
                      Diajukan oleh: <strong>{req.pengaju}</strong> ·{' '}
                      {req.tanggal}
                    </p>
                  </div>
                </div>
              ...
              {showTolakForm === req.id && (
                <div className="bg-red-50 rounded-lg p-3 mb-3">
                  ...
                  <div className="flex gap-2 mt-2">
                    <button className="bg-red-500 text-white text-xs px-3 py-1.5 rounded-lg hover:bg-red-600">
                      Konfirmasi Tolak
                    </button>
                    <button
                      className="text-gray-500 text-xs px-3 py-1.5 rounded-lg hover:bg-gray-100"
                      onClick={() => setShowTolakForm(null)}
                    >
                      Batal
                    </button>
                  </div>
              ...
              <div className="flex gap-2 pt-3 border-t border-gray-100">
                ...
                <button
                  ...
                  onClick={() =>
                    setShowTolakForm(
                      showTolakForm === req.id ? null : req.id
                    )
                  }
                >
                  ❌ Tolak
